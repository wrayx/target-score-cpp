...
ImageAlignment::ImageAlignment(std::string input_img_path,
    ...
    if (util::imageExists(input_img_path) != 0 or
        util::imageExists(reference_img_path) != 0) {
        return;
    }
...
int ImageAlignment::orbFeatureExtractionAlignment() {
    ...
    std::vector<cv::Point2f> input_img_matched_keypoints,
        ref_img_matched_keypoints;
    // matcher.match(input_img_descriptors, reference_img_descriptors, matches);
    matcher.knnMatch(input_img_descriptors, reference_img_descriptors, matches,
                     2);

    // ratio test
    for (unsigned i = 0; i < matches.size(); i++) {
        if (matches[i][0].distance < MATCH_RATIO * matches[i][1].distance) {
            input_img_matched_keypoints.push_back(
                input_img_keypoints[matches[i][0].queryIdx].pt);
            ref_img_matched_keypoints.push_back(
                reference_img_keypoints[matches[i][0].trainIdx].pt);
    ...
    if (good_matches.size() >= 4) {
        homography = cv::findHomography(input_img_matched_keypoints,
                                        ref_img_matched_keypoints, cv::RANSAC);
    } else {
        return -1;
    }
    ...
    cv::drawMatches(input_img, input_img_keypoints, reference_img,
                    reference_img_keypoints, matches, match_img);

    cv::drawMatches(input_img, input_img_keypoints, reference_img,
                    reference_img_keypoints, good_matches, good_match_img);
...
int ImageAlignment::outlineShapeAlignment() {
    ...
    cv::findContours(input_img_thresh, input_img_contours, cv::RETR_LIST,
    ...
    for (size_t idx = 0; idx < square_contours.size(); idx++) {
        ...
        if (hierarchy[idx][PARENT_CONTOUR] == 1) {
            ...
            // util::drawPolyDP(input_img, board_corners, util::RED, 30);
        ...
        // cv::drawContours(quadrilaterals_plot, square_contours, idx,
        // util::WHITE, 1);
    }
    // cv::imshow("shapes", input_img);
    // cv::imwrite("../output_new/detect_square_5.png", input_img);
