...
void drawPolyDP(cv::Mat &img, std::vector<cv::Point> &approximation,
                cv::Scalar color = RED, int thickness) {
    // Iterate over each segment and draw it
    auto itp = approximation.begin();
    while (itp != (approximation.end() - 1)) {
        cv::line(img, *itp, *(itp + 1), color, thickness);
        ++itp;
    }
    // last point linked to first point
    cv::line(img, *(approximation.begin()), *(approximation.end() - 1), color,
             thickness);
}
